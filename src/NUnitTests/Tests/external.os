...
Процедура ПередЗапускомТестов()
	...
	Если НЕ ЗначениеЗаполнено(ПутьККомпоненте) Тогда
		ПутьККомпоненте = ОбъединитьПути(ТекущийКаталог, "src", "deflator", "bin");
		ПутьККомпоненте = ОбъединитьПути(ПутьККомпоненте, "Debug", "net452", "deflator.dll");
	КонецЕсли;
...
Процедура ТестДолжен_РаспаковатьФайл() Экспорт
	...
	ПутьКИсходномуФайлу = ОбъединитьПути(ТекущийКаталог, "test", ИмяТестовогоФайла);
	ПутьКТестовомуФайлу = ПутьКИсходномуФайлу + ".out";
